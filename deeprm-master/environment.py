...
class Env:
    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
        ...
        # initialize system
        self.machine = Machine(pa) #current scheduled jobs
        self.job_slot = JobSlot(pa) #new jobs
        self.job_backlog = JobBacklog(pa) #waiting jobs
        self.job_record = JobRecord() #record of all jobs
        self.extra_info = ExtraInfo(pa) #additional info like time since last new job
    ...
    def get_action_space(self):
        return self.pa.num_nw + 1  # num job slots + 1 for void action


    def is_done(self):
        return self.get_time() >= self.pa.simu_len  # or other termination logic
...
class Machine:
    def __init__(self, pa):
        ...
        step = 1.0 / pa.job_num_cap
        self.colormap = np.arange(step, 1.0, step)
