...
def dqn_learning(env,
    ...
    obs_t = torch.tensor(...).to(device)

    print("Model Q is on device:", next(Q.parameters()).device)
    ...
    reward_log_filename = f"reward_log_{time.strftime('%Y%m%d_%H%M%S')}.csv"

    # Start of training loop
    with open(reward_log_filename, "w") as f:
        f.write("step,reward\n")
    ...
    for t in itertools.count():
        ...
        if t < learning_starts:
            ...
            if sample > threshold:
                obs_tensor = torch.from_numpy(observations).unsqueeze(0).float().to(device) / 255.0
                q_values = Q(obs_tensor)
                action = q_values.max(1)[1].item()
            else:
                action = np.random.randint(num_actions)

        #=== Take step AFTER action is chosen ===
        ...
        if t % 100 == 0:
            print(f"[Step {t}] Current reward: {reward}, Total episode reward: {total_episode_reward}")
        ...
        if done:
            ...
            with open("reward_log.csv", "a") as f:
                f.write(f"{t},{total_episode_reward}\n")
...
'''
            ...
            obs_t = torch.from_numpy(obs_t).float().to(device) / 255.0
            act_t = torch.from_numpy(act_t).long().to(device)
            rew_t = torch.from_numpy(rew_t).float().to(device)
            obs_tp1 = torch.from_numpy(obs_tp1).float().to(device) / 255.0
        ...
        if t % SAVE_MODEL_EVERY_N_STEPS == 0:
            ...
            timestamp = time.strftime("%Y-%m-%d_%H-%M-%S")  # Safe format: no colons
            model_save_path = os.path.join("models", f"{env_id}_double_{int(double_dqn)}_{timestamp}.model")
